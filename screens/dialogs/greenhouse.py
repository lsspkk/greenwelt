# Greenhouse dialog - pick up and drop plants at the greenhouse
import pygame
import random
...
class GreenhouseScreen:
    """
    Full screen greenhouse dialog for picking up and dropping plants.
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        self.fade_timer = 0.0
        self.fade_duration = 2.0  # 2 seconds to fade
        self.background_alpha = 255  # Start at full opacity
        ...
        # Reference to the inventory (set when opened)
    ...
    def _load_plant_list(self):
        ...
        try:
            ...
            debug.info(f"Loaded {len(self.all_plants)} plant types for greenhouse")
        except Exception as e:
            debug.error(f"Failed to load plant list: {e}")

    def _load_background(self):
        """Load the greenhouse background image."""
        image_path = Path(__file__).parent.parent.parent / "assets" / "images" / f"greenhouse{self.greenhouse_index}.png"

        try:
            self.background_image = pygame.image.load(str(image_path)).convert()
            ...
            debug.info(f"Loaded greenhouse background: greenhouse{self.greenhouse_index}.png")
        except Exception as e:
    ...
    def open(self, player_inventory: Dict[str, int], pick_callback, drop_callback, can_pick_callback):
        """
        Open the greenhouse dialog.

        Args:
            player_inventory: Reference to the player's inventory dict
    ...
    def _draw_plant_grid(self):
        ...
        down_text_rect = down_text.get_rect(center=self.scroll_down_rect.center)
        ...
        for row in range(self.visible_rows):
            for col in range(self.plants_per_row):
                ...
                plus_color = self.button_color if can_pick else (50, 50, 60)
                pygame.draw.rect(self.screen, plus_color, plus_rect, border_radius=8)
                ...
                # Minus button
                minus_rect = pygame.Rect(item_x + btn_width + 10, btn_y, btn_width, btn_height)
                # Check if player has this plant
                has_plant = False
                ...
                minus_color = self.button_color if has_plant else (50, 50, 60)
                pygame.draw.rect(self.screen, minus_color, minus_rect, border_radius=8)
    ...
    def _draw_inventory_panel(self):
        ...
        title_text = self.title_font.render(f"Kantamus ({total}/15)", True, self.text_color)
        self.screen.blit(title_text, (panel_x + 20, panel_y + 15))
        ...
        if len(self.player_inventory) == 0:
            empty_text = self.text_font.render("Ei kasveja", True, (150, 150, 150))
        ...
        for plant_file in sorted_plants:
            ...
            text_surface = self.text_font.render(entry_text, True, self.text_color)
            ...
            if list_y > panel_y + panel_height - 50:
                more_text = self.small_font.render("...", True, (150, 150, 150))
    ...
    def _draw_close_button(self):
        ...
        pygame.draw.rect(self.screen, self.button_color, self.close_button_rect, border_radius=12)
        pygame.draw.rect(self.screen, self.accent_color, self.close_button_rect, 3, border_radius=12)

        close_text = self.button_font.render("Sulje", True, self.text_color)
